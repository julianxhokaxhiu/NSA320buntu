...
REQUIREDPACKAGES=( git-core debootstrap asciidoc docbook-xsl curl build-essential debhelper autoconf automake autotools-dev curl subversion unzip squashfs-tools cdbs po4a python-utidylib germinate lzma u-boot-tools binfmt-support qemu-kvm-extras-static ) # libterm-readline-gnu-perl
NOTINSTALLED=()
...
if ls $THISDIR/binary*.tgz > /dev/null 2>&1 ; then
	rm $THISDIR/binary*.tgz
fi
